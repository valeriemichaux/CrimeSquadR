...
importances.geschlecht %>%
  ggplot(aes(x = Total, y = fct_reorder(Attribute, Total))) + geom_col(fill = "lightblue") +
...
pwu%>%
 ...
 stat_summary(geom="text", aes(label=sprintf("%1.2f%%", ..y..)), size=3) +
  coord_flip()+
   scale_x_continuous(limits = c(-100, 100), breaks = c(-100,-50,0,50,100)) +
  labs(x="Level", y="Teilnutzenwerte der Merkmale")+
